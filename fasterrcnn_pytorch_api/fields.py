            ...
            for k2, v2 in v1.items():
                if k2 == "p":
                    ...
                    if not isinstance(v2, int) or v2 < 0:
                        raise ValidationError(
                            f"Invalid value for '{k2}' in {k1}: {v2}. It must be a non-negative integer."
                        )
                    ...
                    if not isinstance(v2, int) or not isinstance(v2, (int, int)):
                        raise ValidationError(
                            f"Invalid value for '{k2}': {v2}. It must be an int or (int, int)."
                        )
...
class TrainArgsSchema(Schema):
    ...
    model = fields.Str(
        enum=configs.BACKBONES, required=True, description="Name of the model."
    )

    data_config = fields.Str(required=True, description="Path to the data config file.")

    use_train_aug = fields.Bool(
        required=False,
        missing=False,
        enum=[True, False],
        description="Whether to use train augmentation, uses some advanced augmentation that may make training difficult when used with mosaic. If true, it uses the options in aug_training_option. You can change that to have custom augmentation.",
    )
    ...
    epochs = fields.Int(
        required=False, missing=4, description="Number of epochs to train."
    )
    ...
    batch = fields.Int(
        required=False, missing=4, description="Batch size to load the data."
    )

    lr = fields.Float(
        required=False, missing=0.001, description="Learning rate for training."
    )

    imgsz = fields.Int(
        required=False, missing=640, description="Image size to feed to the network."
    )
    ...
    weights = fields.Str(
        required=False,
        missing=None,
        description='Path to model weights if using custom pretrained weights. The name of the directory that contains the checkpoint within the "model" directory.',
    )
    ...
    resume_training = fields.Bool(
        ...
        missing=False,
        enum=[True, False],
        description="If using custom pretrained weights, resume training from the last step of the provided checkpoint. If True, the path to the weights should be specified in the argument weights.",
    )
    ...
    disable_wandb = fields.Bool(
        required=False, missing=True, description="Whether to use WandB for logging."
    )

    seed = fields.Int(
        required=False, missing=0, description="Global seed for training."
    )
...
class PredictArgsSchema(Schema):
    ...
    timestamp = fields.Str(
        required=False,
        missing=None,
        description="Model timestamp to be used for prediction. To see the available timestamp, please run the get_metadata function. If no timestamp is given, the model will be loaded from COCO.",
    )
    ...
    model = fields.Str(
        ...
        description='Please provide the name of the model you want to use for inference. If you have specified neither timestamp nor model name, the default model "fasterrcnn_resnet50_fpn_v2" is loaded.',
    )

    threshold = fields.Float(
        required=False, missing=0.5, description="Detection threshold."
    )
    ...
    accept = fields.Str(
        ...
        location="headers",
        validate=validate.OneOf(["application/json", "image/png", "video/mp4"]),
        description="Returns a PNG file with detection results or a JSON with the prediction.",
    )
